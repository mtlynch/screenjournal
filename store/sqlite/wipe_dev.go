...
func (s Store) Clear() {
	...
	if _, err := s.exec(`DELETE FROM movies`); err != nil {
		log.Fatalf("failed to delete movies: %v", err)
	}
	if _, err := s.exec(`DELETE FROM reviews`); err != nil {
	...
	if _, err := s.exec(`DELETE FROM invites`); err != nil {
		log.Fatalf("failed to delete invites: %v", err)
	}
	if _, err := s.exec(`DELETE FROM notification_preferences`); err != nil {
